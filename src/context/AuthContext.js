...
function AuthContextProvider({ children }) {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            // als er GEEN token is, doen we niets, en zetten we de status op 'done'
            toggleIsAuth({
                isAuth: false,
                user: null,
                status: 'done',
    ...
    function login(JWT) {
        // zet de token in de Locale Storage
    ...
    async function fetchUserData(id, token, redirectUrl) {
        try {
            ...
            // als er een redirect URL is meegegeven (bij het mount-effect doen we dit niet) linken we hier naar toe door
